...
# --------------------------assignments_test--------------------------------\
# Generates an archive of the source code compressed as a .zip file.
data "archive_file" "forecast_assignments_test" {
  type        = "zip"
  source_dir  = "../../../cloud_functions/forecast/assignments_test"
  output_path = "/tmp/forecast_assignments_test.zip"
}

# Add source code zip to the Cloud Function's bucket
resource "google_storage_bucket_object" "forecast_assignments_test" {
  source       = data.archive_file.forecast_assignments_test.output_path
  content_type = "application/zip"

  # Append to the MD5 checksum of the files's content
  # to force the zip to be updated as soon as a change occurs
  name   = "cloud_function-${data.archive_file.forecast_assignments_test.output_md5}.zip"
  bucket = data.google_storage_bucket.function_bucket.name
}

resource "google_cloudfunctions_function" "forecast_assignments_test" {
  name                = "forecast_assignments_test_pipe"
  runtime             = var.function_runtime
  available_memory_mb = 2048
  timeout             = 540
  # Get the source code of the cloud function as a Zip compression
  source_archive_bucket = data.google_storage_bucket.function_bucket.name
  source_archive_object = google_storage_bucket_object.forecast_assignments_test.name
  ...
  environment_variables = {
    "DATASET_ID"           = google_bigquery_dataset.forecast_raw.dataset_id
    "TABLE_NAME"           = google_bigquery_table.forecast_assignments_test.table_id
    "TABLE_LOCATION"       = google_bigquery_dataset.forecast_raw.location
    "GOOGLE_CLOUD_PROJECT" = var.project
